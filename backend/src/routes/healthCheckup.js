...
const ensureHealthCheckupSchema = async (db = pool) => {
  await db.query(`
...
const activityLogInitPromise = process.env.NODE_ENV === 'test'
  ? Promise.resolve()
  : Promise.all([ensureActivityLogTable(), ensureHealthCheckupSchema()]).catch((e) => {
...
router.post('/', async (req, res) => {
  ...
  const normalizedPmno = cleanText(pmno);
  if (normalizedPmno && normalizedPmno.length > 20) {
    return res.status(400).json({ error: 'PM number must be 20 characters or less' });
  }
  const client = await pool.connect();
  try {
    await activityLogInitPromise;
    await client.query('BEGIN');
    await ensureHealthCheckupSchema(client);
    ...
    const { rows } = await client.query(
      ...
      [normalizedPmno,serial,model,make,sapno,mesno,dpi,firmware,km,loftware,ip,mac,loc,stage,bay,wc,health||'ok',issue_desc,req_parts,JSON.stringify(damaged_parts||[]),is_repeat||false,engineer]
    );
